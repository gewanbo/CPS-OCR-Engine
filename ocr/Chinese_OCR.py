...
tf.app.flags.DEFINE_string('app_log_dir', './log', 'the logging dir')
...
def build_graph(top_k):
    keep_prob = tf.compat.v1.placeholder(dtype=tf.float32, shape=[], name='keep_prob')  # dropout打开概率
    ...
    with tf.device('/cpu:0'):
        ...
        global_step = tf.compat.v1.get_variable("step", [], initializer=tf.constant_initializer(0.0), trainable=False)
...
def train():
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        train_writer = tf.compat.v1.summary.FileWriter(FLAGS.app_log_dir + '/train', sess.graph)
        test_writer = tf.compat.v1.summary.FileWriter(FLAGS.app_log_dir + '/val')
        start_step = 0
        # 可以从某个step下的模型继续训练
        if FLAGS.restore:
